...
export const ChatMessageBubble: React.FC<ChatMessageBubbleProps> = ({
  ...
  const firstRow =
    message.result && message.result.results.length > 0
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Query Result Block (for assistant messages with results) */}
        {!isUser && message.result && showResult && (
          <div className="mt-4 w-full max-w-none lg:max-w-4xl">
            ...
            <div className="mb-4 overflow-x-auto rounded-lg border border-slate-200 bg-white">
              {message.result.results.length === 0 ? (
                ...
                <table className="w-full min-w-[560px] text-xs">
                  ...
                  <tbody>
                    {message.result.results.map((row, rowIdx) => (
                      <tr
                        key={rowIdx}
            ...
            <div className="text-xs text-slate-600 bg-slate-50 px-3 py-2 rounded-md border border-slate-200">
              <span className="font-medium">Returned {message.result.results.length} row(s)</span>
            </div>
          </div>
        )}
      </div>
